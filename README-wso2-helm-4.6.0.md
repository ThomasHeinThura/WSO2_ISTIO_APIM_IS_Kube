# APIM 4.6.0 (CP + Universal GW) on Kind + Istio (WSO2 Helm charts)

This guide uses **only** the official WSO2 Helm chart repo (`https://helm.wso2.com`) and keeps the values files close to the WSO2 reference defaults.
...
- Istio base + istiod already installed.
- External MySQL is reachable at `wso2sql.orb.local:3306` with `root / 1qaz!QAZ`.
...
## 2) Initialize MySQL schemas (APIM)
This uses the SQL in `WSO2_ISTIO_APIM_IS_Kube/mysql-scripts` (creates `WSO2AM_DB`, `WSO2AM_SHARED_DB` and grants `wso2carbon/wso2carbon`).
